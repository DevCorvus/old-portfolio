...
export default function Skill({ children, title, level = 0 }: SkillInterface) {
  ...
  const { skills: { levels } } = useTranslation();
  ...
  return (
    <div>
      {hasDetails ? (
        <button onClick={() => setShowDetails(prevState => !prevState)} className='group w-full flex justify-between items-center' type='button'>
          <div className='flex items-center gap-2 transition group-hover:text-violet-300'>
            <div className={`transition-all duration-300 ${showDetails ? 'rotate-180' : 'rotate-0'}`}>
      ...
      {showDetails && (
        ...
        {transitions((styles, item) => item && (
          <animated.p style={styles}>{children}</animated.p>
        ))}
        </>
      )}
    </div>
