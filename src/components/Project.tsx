...
export default function Project({ name, description, favicon, image, url }: ProjectInterface) {
  return (
      ...
      <div className='flex flex-col justify-between bg-slate-100 rounded-md border border-violet-900'>
        <div className='text-neutral-900 p-2'>
          <header className='flex items-center gap-2'>
            <Image src={favicon} width={30} height={30} alt={`${name} favicon`} />
            <h2 className='text-xl font-bold'>{name}</h2>
          </header>
          <p>{description}</p>
        </div>
        <a className='relative group' href={url} target='_blank' rel='noopener noreferrer'>
          ...
          <Image className='rounded-md transition-all duration-300 group-hover:scale-125 group-hover:opacity-25' src={image} layout='intrinsic' alt='Relyn App' />
